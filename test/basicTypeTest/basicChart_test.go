...
func Test_Chart_UpLoad_DataType(t *testing.T) {
	Convey("Test_Chart_upload:", t, func() {
		...
		Convey("Test_chart_upload:", func() {
			dtl, err := model.NewDataTypeListWithRaw(model.DtString, []string{"chart", "xaxis", "yaxis"})
			So(err, ShouldBeNil)
			So(dtl.DataType(), ShouldEqual, model.DtString)
			dl, err := model.NewDataTypeListWithRaw(model.DtString, []string{"chart", "xaxis", "yaxis"})
			So(err, ShouldBeNil)
			...
			d, err := model.NewDataTypeListWithRaw(model.DtInt, []int32{1, 2, 3, 4, 5})
			So(err, ShouldBeNil)
			rl, err := model.NewDataTypeListWithRaw(model.DtInt, []int32{1, 2, 3, 4, 5})
			So(err, ShouldBeNil)
			cl, err := model.NewDataTypeListWithRaw(model.DtInt, []int32{1})
			So(err, ShouldBeNil)
			...
			So(re.Get(0).String(), ShouldEqual, data.String())
			So(re.Get(1).String(), ShouldEqual, st.String())
			So(re.Get(2).String(), ShouldEqual, ct.String())
			So(re.Get(3).String(), ShouldEqual, ti.String())
			fmt.Print(re)
			So(err, ShouldBeNil)
