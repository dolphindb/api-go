...
func Test_Set_DownLoad_DataType_boundary(t *testing.T) {
	...
	Convey("Test_set_boundary:", t, func() {
		Convey("Test_set_boundary_nil_vector:", func() {
			_, err := model.NewSet(nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "set vector must not be nil")
		})

		Convey("Test_set_boundary_type_function:", func() {
			dtl, err := model.NewDataTypeListFromRawData(model.DtFunction, []string{"symbol"})
			So(err, ShouldBeNil)
			set, err := model.NewSet(model.NewVector(dtl))
			So(err, ShouldBeNil)
			So(set.Rows(), ShouldEqual, 1)
			So(set.GetDataTypeString(), ShouldEqual, "function")
		})
...
func Test_Set_DownLoad_DataType_char(t *testing.T) {
	...
	Convey("Test_set_char:", t, func() {
		...
		Convey("Test_set_char_not_null:", func() {
			...
			result := s.(*model.Set)
			re := result.Vector.Data.Value()
			So(re[1], ShouldEqual, 'a')
			So(re[1], ShouldEqual, 97)
			So(result.Vector.RowCount, ShouldEqual, 4)
			reType := result.GetDataType()
		...
		Convey("Test_set_char_has_null:", func() {
			...
			result := s.(*model.Set)
			re := result.Vector.Data.Value()
			So(result.Vector.Data.IsNull(1), ShouldBeTrue)
			So(re[2], ShouldEqual, 25)
			So(result.Vector.RowCount, ShouldEqual, 4)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 2)
			reTypeString := result.GetDataTypeString()
...
func Test_Set_UpLoad_DataType_int(t *testing.T) {
	...
	Convey("Test_set_int_upload:", t, func() {
		...
		Convey("Test_set_int:", func() {
			...
			set, err := model.NewSet(model.NewVector(data))
			So(err, ShouldBeNil)
...
func Test_Set_UpLoad_DataType_big_array(t *testing.T) {
	...
	Convey("Test_set_big_array_upload:", t, func() {
		...
		So(err, ShouldBeNil)
		set, err := model.NewSet(model.NewVector(col))
		So(err, ShouldBeNil)
		_, err = db.Upload(map[string]model.DataForm{"s": set})
		So(err, ShouldBeNil)
		res, _ := db.RunScript("s")
