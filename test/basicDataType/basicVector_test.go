...
func Test_Vector_UpLoad_int_array_vector(t *testing.T) {
	...
	Convey("Test_Vector_int_array_vector:", t, func() {
		...
		s, err := model.NewVectorWithArrayVector(NewData)
		So(err, ShouldBeNil)
		_, err = db.Upload(map[string]model.DataForm{"s": s})
		So(err, ShouldBeNil)
...
func Test_Vector_UpLoad_array_vector_boundary(t *testing.T) {
	t.Parallel()
	Convey("Test_Vector_array_vector_boundary:", t, func() {
		Convey("Test_vector_array_vector_empty:", func() {
			_, err := model.NewVectorWithArrayVector(nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "array vector data must not be empty")
		})

		Convey("Test_vector_array_vector_unsupported_type:", func() {
			_, err := model.NewVectorWithArrayVector([]*model.ArrayVector{})
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "array vector data must not be empty")

			strv, err := model.NewDataTypeListFromRawData(model.DtString, []string{"hello"})
			So(err, ShouldBeNil)
			av := model.NewArrayVector([]*model.Vector{model.NewVector(strv)})
			_, err = model.NewVectorWithArrayVector(av)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "array vector does not support datatype")
		})
	})
}
...
func Test_Vector_UpLoad_vector_decimal64_scale_12(t *testing.T) {
	...
	Convey("Test_Vector_array_vector:", t, func() {
		...
		s, err := model.NewVectorWithArrayVector(result)
		So(err, ShouldBeNil)
		_, err = db.Upload(map[string]model.DataForm{"s": s})
		So(err, ShouldBeNil)
		res, err := db.RunScript("s")
...
func Test_GetRawValue_UpLoad_array_vector_decimal32(t *testing.T) {
	...
	Convey("Test_Vector_array_vector:", t, func() {
		...
		So(err, ShouldBeNil)
		re := model.NewVector(vec1)
		m1, _ := model.NewDataType(model.DtDecimal32, &model.Decimal32{Scale: 3, Value: 2.366})
		m2, _ := model.NewDataType(model.DtDecimal32, &model.Decimal32{Scale: 3, Value: -5.692})
		...
		re = res.(*model.Vector)
		m := re.String()
		So(m, ShouldEqual, "vector<decimal32Array>([[2.366, , -5.692], [2.366, , -5.692], [, , ]])")
		result1 := re.GetVectorValue(0)
		for i := 0; i < vec1.Len(); i++ {
...
func Test_GetRawValue_UpLoad_array_vector_decimal64(t *testing.T) {
	...
	Convey("Test_Vector_array_vector:", t, func() {
		...
		re := model.NewVector(vec1)
		fmt.Println("re", re)
		m1, _ := model.NewDataType(model.DtDecimal64, &model.Decimal64{Scale: 3, Value: 2.366})
		m2, _ := model.NewDataType(model.DtDecimal64, &model.Decimal64{Scale: 3, Value: -5.692})
		So(re.GetRawValue()[0], ShouldResemble, model.NewScalar(m1).Value())
		...
		m := re.String()
		fmt.Println("m", m)
		So(m, ShouldEqual, "vector<decimal64Array>([[2.366, , -5.692], [2.365, , -5.691], [, , ]])")
		result1 := re.GetVectorValue(0)
		for i := 0; i < vec1.Len(); i++ {
			So(result1.GetRawValue()[i], ShouldResemble, vec1.Value()[i])
		}
		result2 := re.GetVectorValue(1)
		vec2M, _ := model.NewDataTypeListFromRawData(model.DtDecimal64, &model.Decimal64s{Scale: 3, Value: []float64{2.365, model.NullDecimal64Value, -5.691}})
...
func Test_Vector(t *testing.T) {
	Convey("Test_vector", t, func() {
		Convey("Test_array_vector", func() {
			vct, err := model.NewVectorWithArrayVector(nil)
			So(err, ShouldNotBeNil)
			So(vct, ShouldBeNil)
