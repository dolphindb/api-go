...
func Test_Pair_DownLoad_int(t *testing.T) {
	...
	Convey("Test_pair_int:", t, func() {
		...
		Convey("Test_pair_int_not_null:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, -1024)
			So(result.Vector.Get(1).Value(), ShouldEqual, 1048576)
		...
		Convey("Test_pair_int_pre_one_nll:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(1).Value(), ShouldEqual, 1048576)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 4)
			reTypeString := result.GetDataTypeString()
		...
		Convey("Test_pair_int_last_one_nll:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(1).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(0).Value(), ShouldEqual, 1)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 4)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "int")
...
func Test_Pair_DownLoad_DataType_boundary(t *testing.T) {
	...
	Convey("Test_pair_boundary:", t, func() {
		Convey("Test_pair_boundary_nil_vector:", func() {
			_, err := model.NewPair(nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "pair vector must not be nil")
		})

		Convey("Test_pair_boundary_vector_length_not_two_1:", func() {
			dtl, err := model.NewDataTypeListFromRawData(model.DtString, []string{"only-one"})
			So(err, ShouldBeNil)
			_, err = model.NewPair(model.NewVector(dtl))
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "the vector must be of length 2 when initializing a pair")
		})

		Convey("Test_pair_boundary_vector_length_not_two_3:", func() {
			dtl, err := model.NewDataTypeListFromRawData(model.DtString, []string{"only-one", "only-one", "only-one"})
			So(err, ShouldBeNil)
			_, err = model.NewPair(model.NewVector(dtl))
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "the vector must be of length 2 when initializing a pair")
		...
		Convey("Test_pair_boundary_special_characters:", func() {
			...
			So(err, ShouldBeNil)
			pair, err := model.NewPair(model.NewVector(dtl))
			So(err, ShouldBeNil)
		...
		Convey("Test_pair_boundary_empty_values:", func() {
			dtl, err := model.NewDataTypeListFromRawData(model.DtString, []string{"", ""})
			So(err, ShouldBeNil)
			pair, err := model.NewPair(model.NewVector(dtl))
			So(err, ShouldBeNil)
			So(pair.GetDataTypeString(), ShouldEqual, "string")
			So(pair.Vector.Get(0).Value(), ShouldEqual, "")
			So(pair.Vector.Get(1).Value(), ShouldEqual, "")
		})
...
func Test_Pair_DownLoad_string(t *testing.T) {
	...
	Convey("Test_pair_string:", t, func() {
		...
		Convey("Test_pair_string_not_null:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, "hello")
			So(result.Vector.Get(1).Value(), ShouldEqual, "world")
			reType := result.GetDataType()
		...
		Convey("Test_pair_string_pre_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(0).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(1).Value(), ShouldEqual, "theworld")
			reType := result.GetDataType()
			So(reType, ShouldEqual, 18)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "string")
		...
		Convey("Test_pair_string_last_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(1).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(0).Value(), ShouldEqual, "thehello")
			reType := result.GetDataType()
			So(reType, ShouldEqual, 18)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "string")
...
func Test_Pair_DownLoad_char(t *testing.T) {
	...
	Convey("Test_pair_char:", t, func() {
		...
		Convey("Test_pair_char_not_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, 1)
			So(result.Vector.Get(1).Value(), ShouldEqual, 124)
			reType := result.GetDataType()
		...
		Convey("Test_pair_char_last_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(1).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(0).Value(), ShouldEqual, 16)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 2)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "char")
...
func Test_Pair_DownLoad_bool(t *testing.T) {
	...
	Convey("Test_pair_bool:", t, func() {
		...
		Convey("Test_pair_bool_not_null:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, true)
			So(result.Vector.Get(1).Value(), ShouldEqual, false)
			reType := result.GetDataType()
...
func Test_Pair_DownLoad_short(t *testing.T) {
	...
	Convey("Test_pair_short:", t, func() {
		...
		Convey("Test_pair_short:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, -2)
			So(result.Vector.Get(1).Value(), ShouldEqual, 1024)
			reType := result.GetDataType()
		...
		Convey("Test_pair_short_pre_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(0).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(1).Value(), ShouldEqual, 31689)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 3)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "short")
		...
		Convey("Test_pair_short_last_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(1).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(0).Value(), ShouldEqual, -15225)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 3)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "short")
...
func Test_Pair_DownLoad_long(t *testing.T) {
	...
	Convey("Test_pair_long:", t, func() {
		...
		Convey("Test_pair_long_last_one_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(1).IsNull(), ShouldBeTrue)
			So(result.Vector.Get(0).Value(), ShouldEqual, -1048576)
			reType := result.GetDataType()
			So(reType, ShouldEqual, 5)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "long")
...
func Test_Pair_DownLoad_double(t *testing.T) {
	...
	Convey("Test_pair_double:", t, func() {
		...
		Convey("Test_pair_double_not_null:", func() {
			...
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, -2.0)
			So(result.Vector.Get(1).Value(), ShouldEqual, 1048576)
			reType := result.GetDataType()
...
func Test_Pair_DownLoad_float(t *testing.T) {
	...
	Convey("Test_pair_float:", t, func() {
		...
		Convey("Test_pair_float_not_null:", func() {
			...
			result := s.(*model.Pair)
			So(result.Vector.Get(0).Value(), ShouldEqual, -2.0)
			So(result.Vector.Get(1).Value(), ShouldEqual, 1048576.0)
			reType := result.GetDataType()
...
func Test_Pair_DownLoad_date(t *testing.T) {
	...
	Convey("Test_pair_date:", t, func() {
		...
		Convey("Test_pair_date_not_null:", func() {
			...
			So(result.Vector.Get(0).Value(), ShouldEqual, zx[0])
			So(result.Vector.Get(1).Value(), ShouldEqual, zx[1])
			reType := result.GetDataType()
...
func Test_Pair_DownLoad_decimal128(t *testing.T) {
	t.Parallel()
	Convey("Test_pair_decimal128:", t, func() {
		db, err := api.NewSimpleDolphinDBClient(context.TODO(), setup.Address, setup.UserName, setup.Password)
		So(err, ShouldBeNil)
		Convey("Test_pair_decimal128_not_null:", func() {
			s, err := db.RunScript("a=decimal128('-1.123123123123123123123123123123123123123',28):decimal128(`0,28);a")
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			val := result.Vector.Data.Value()
			for i := 0; i < len(val); i++ {
				decimal128Val := val[i].(*model.Decimal128)
				So(decimal128Val.Scale, ShouldEqual, 28)
			}
			So(result.Vector.Data.StringList()[0], ShouldEqual, "-1.1231231231231231231231231231")
			So(result.Vector.Data.StringList()[1], ShouldEqual, "0.0000000000000000000000000000")
			reType := result.GetDataType()
			So(reType, ShouldEqual, model.DtDecimal128)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "decimal128")
		})
		Convey("Test_pair_decimal128_pre_one_null :", func() {
			s, err := db.RunScript("a=(decimal128(NULL,0):decimal128(1.4,0));a")
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			val := result.Vector.Data.Value()
			for i := 0; i < len(val); i++ {
				decimal128Val := val[i].(*model.Decimal128)
				So(decimal128Val.Scale, ShouldEqual, 0)
			}
			So(result.Vector.Data.StringList()[0], ShouldEqual, "")
			So(result.Vector.Data.StringList()[1], ShouldEqual, "1")
			reType := result.GetDataType()
			So(reType, ShouldEqual, model.DtDecimal128)
			reTypeString := result.GetDataTypeString()
			So(reTypeString, ShouldEqual, "decimal128")
		})
		Convey("Test_pair_decimal128_last_one_null:", func() {
			s, err := db.RunScript("a=(decimal128(-1.123,0):decimal128(NULL,0));a")
			So(err, ShouldBeNil)
			result := s.(*model.Pair)
			val := result.Vector.Data.Value()
			for i := 0; i < len(val); i++ {
			...
			So(result.Vector.Data.StringList()[0], ShouldEqual, "-1")
			So(result.Vector.Data.StringList()[1], ShouldEqual, "")
			reType := result.GetDataType()
			So(reType, ShouldEqual, model.DtDecimal128)
			reTypeString := result.GetDataTypeString()
...
func Test_Pair_UpLoad_int(t *testing.T) {
	...
	Convey("Test_pair_int_upload:", t, func() {
		...
		Convey("Test_pair_int:", func() {
			...
			pair, err := model.NewPair(model.NewVector(data))
			So(err, ShouldBeNil)
...
func Test_Pair_UpLoad_decimal32(t *testing.T) {
	...
	Convey("Test_pair_decimal32_upload:", t, func() {
		...
		Convey("Test_pair_decimal32:", func() {
			data, _ := model.NewDataTypeListFromRawData(model.DtDecimal32, &model.Decimal32s{Scale: 2, Value: []float64{1.32157, model.NullDecimal32Value}})
			pair, err := model.NewPair(model.NewVector(data))
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": pair})
			So(err, ShouldBeNil)
...
func Test_Pair_UpLoad_decimal128(t *testing.T) {
	t.Parallel()
	Convey("Test_pair_decimal128_upload:", t, func() {
		db, err := api.NewSimpleDolphinDBClient(context.TODO(), setup.Address, setup.UserName, setup.Password)
		So(err, ShouldBeNil)
		Convey("Test_pair_decimal128:", func() {
			data, _ := model.NewDataTypeListFromRawData(model.DtDecimal128, &model.Decimal128s{30, []string{"-1.123123123123123123123123123123123123123", model.NullDecimal128Value}})
			pair, err := model.NewPair(model.NewVector(data))
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": pair})
			So(err, ShouldBeNil)
			_, err = db.RunScript("assert eqObj(s, decimal128('-1.123123123123123123123123123123123123123':NULL, 30))")
			So(err, ShouldBeNil)
			r1, _ := db.RunScript("s")
			r2, _ := db.RunScript("decimal128('-1.123123123123123123123123123123123123123':NULL, 30)")
			fmt.Println(r1.(*model.Pair).Vector)
			fmt.Println(r2.(*model.Pair).Vector)
			So(err, ShouldBeNil)
			ty, err := db.RunScript("typestr(s)")
			So(err, ShouldBeNil)
			So(ty.String(), ShouldEqual, "string(DECIMAL128 PAIR)")
		})
		So(db.Close(), ShouldBeNil)
	})
}
...
func Test_Pair_huge_val(t *testing.T) {
	t.Parallel()
	Convey("Test_pair_big_string_symbol_blob:", t, func() {
		db, err := api.NewSimpleDolphinDBClient(context.TODO(), setup.Address, setup.UserName, setup.Password)
		So(err, ShouldBeNil)
		Convey("Test_pair_big_string:", func() {
			res, _ := db.RunScript("a=string(concat(take(\"123&#@!^%;d《》中文\",100000)));x= pair(a, a);x")

			result := res.(*model.Pair)
			// fmt.Println(result.Vector.Get(0).String())
			So(result.GetDataTypeString(), ShouldEqual, "string")
			So(result.Vector.Get(0).String(), ShouldEqual, strings.Repeat("123&#@!^%;d《》中文", 100000))
			So(result.Vector.Get(1).String(), ShouldEqual, strings.Repeat("123&#@!^%;d《》中文", 100000))
		...
		Convey("Test_pair_big_blob:", func() {
			res, _ := db.RunScript("a=blob(concat(take(\"123&#@!^%;d《》中文\",10)));x= pair(a, a);x")

			result := res.(*model.Pair)
			// fmt.Println(result.Vector.Get(0).String())
			So(result.GetDataTypeString(), ShouldEqual, "blob")
			So(result.Vector.Get(0).String(), ShouldEqual, strings.Repeat("123&#@!^%;d《》中文", 10))
			So(result.Vector.Get(1).String(), ShouldEqual, strings.Repeat("123&#@!^%;d《》中文", 10))
			_, err = db.Upload(map[string]model.DataForm{"b": result})
			So(err, ShouldBeNil)
			ans, _ := db.RunScript("eqObj(x,b)")
			So(ans.(*model.Scalar).Value(), ShouldBeTrue)
		})
		Convey("Test_pair_big_symbol:", func() {
			res, err := db.RunScript("a=symbol([concat(take(\"123&#@!^%;d《》中文\",100000))])[0];x= pair(a, a);x")
			assert.NoError(t, err)
			result := res.(*model.Pair)
			for i := 0; i < 2; i++ {
				So(result.Vector.Get(i).String(), ShouldEqual, strings.Repeat("123&#@!^%;d《》中文", 100000))
			}
			...
			rawdata, _ := model.NewDataTypeListFromRawData(model.DtSymbol, []string{str, str})
			s := model.NewVector(rawdata)
			p, err := model.NewPair(s)
			So(err, ShouldBeNil)
