...
func Test_Matrix_DownLoad_boundary(t *testing.T) {
	...
	Convey("Test_matrix_boundary:", t, func() {
		...
		Convey("Test_matrix_boundary_nil_data_vector:", func() {
			_, err := model.NewMatrix(nil, nil, nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "matrix data vector must not be nil")
		})
...
func Test_Matrix_UpLoad_DataType_int(t *testing.T) {
	...
	Convey("Test_matrix_int_upload:", t, func() {
		...
		Convey("Test_matrix_int_upload:", func() {
			...
			So(err, ShouldBeNil)
			mtx, err := model.NewMatrix(model.NewVector(data), nil, nil)
			So(err, ShouldBeNil)
...
func Test_Matrix_UpLoad_DataType_complex(t *testing.T) {
	...
	Convey("Test_matrix_complex_upload:", t, func() {
		...
		Convey("Test_matrix_complex_upload:", func() {
			...
			So(err, ShouldBeNil)
			mtx, err := model.NewMatrix(model.NewVector(data), model.NewVector(rl), model.NewVector(cl))
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": mtx})
			So(err, ShouldBeNil)
			res, _ := db.RunScript("s")
