...
func Test_Dictionary_DownLoad_DataType_boundary(t *testing.T) {
	...
	Convey("Test_dictionary_boundary:", t, func() {
		Convey("Test_dictionary_boundary_missing_key:", func() {
			...
			So(err, ShouldBeNil)
			dict, err := model.NewDictionary(model.NewVector(keys), model.NewVector(vals))
			So(err, ShouldBeNil)
		...
		Convey("Test_dictionary_boundary_nil_keys_values:", func() {
			_, err := model.NewDataTypeListFromRawData(model.DtInt, []int32{1, 2})
			So(err, ShouldBeNil)
			_, err = model.NewDictionary(nil, nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "dictionary keys must not be nil")
		})

		Convey("Test_dictionary_boundary_nil_keys:", func() {
			vals, err := model.NewDataTypeListFromRawData(model.DtInt, []int32{1, 2})
			So(err, ShouldBeNil)
			_, err = model.NewDictionary(nil, model.NewVector(vals))
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "dictionary keys must not be nil")
		})

		Convey("Test_dictionary_boundary_nil_values:", func() {
			keys, err := model.NewDataTypeListFromRawData(model.DtString, []string{"A", "B"})
			So(err, ShouldBeNil)
			_, err = model.NewDictionary(model.NewVector(keys), nil)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "dictionary values must not be nil")
		})

		Convey("Test_dictionary_boundary_mismatched_rows:", func() {
			keys, err := model.NewDataTypeListFromRawData(model.DtString, []string{"A", "B"})
			So(err, ShouldBeNil)
			vals, err := model.NewDataTypeListFromRawData(model.DtInt, []int32{1})
			So(err, ShouldBeNil)
			_, err = model.NewDictionary(model.NewVector(keys), model.NewVector(vals))
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "dictionary keys has 2 rows, values has 1 rows")
		})
...
func Test_Dictionary(t *testing.T) {
	Convey("Test_dictionary", t, func() {
		...
		dict, err := model.NewDictionary(nil, val)
		So(err, ShouldNotBeNil)
		So(dict, ShouldBeNil)

		dict, err = model.NewDictionary(val, val)
		So(err, ShouldBeNil)
		dict.Values = nil
