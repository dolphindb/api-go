...
func Test_Table_DownLoad_DataType_boundary(t *testing.T) {
	...
	Convey("Test_table_boundary:", t, func() {
		...
		Convey("Test_table_boundary_mismatched_input:", func() {
			...
			So(err.Error(), ShouldContainSubstring, "The length of colNames, colTypes and colValues should be equal.")
		})

		Convey("Test_table_boundary_mismatched_row_count_from_raw_data:", func() {
			_, err := model.NewTableFromRawData(
				[]string{"sym", "qty"},
				[]model.DataTypeByte{model.DtString, model.DtInt},
				[]interface{}{[]string{"A", "B"}, []int32{1}},
			)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "column \"qty\" has 1 rows, expect 2")
		})

		Convey("Test_table_boundary_mismatched_row_count_from_vector:", func() {
			col1, err := model.NewDataTypeListFromRawData(model.DtString, []string{"A", "B"})
			So(err, ShouldBeNil)
			col2, err := model.NewDataTypeListFromRawData(model.DtInt, []int32{1, 2})
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"sym", "qty"}, []*model.Vector{model.NewVector(col1), model.NewVector(col2)})
			So(err, ShouldBeNil)

			col2Vector := tb.GetColumnByIndex(1)
			appendValue, err := model.NewDataType(model.DtInt, int32(3))
			So(err, ShouldBeNil)
			So(col2Vector.Append(appendValue), ShouldBeNil)

			buf := bytes.NewBuffer(nil)
			wr := protocol.NewWriter(buf)
			err = tb.Render(wr, protocol.LittleEndian)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "column \"qty\" has 3 rows, expect 2")
		})
	})
}
...
func Test_Table_DownLoad_DataType_decimal128(t *testing.T) {
	t.Parallel()
	Convey("Test_Table_with_decimal128:", t, func() {
		db, err := api.NewSimpleDolphinDBClient(context.TODO(), setup.Address, setup.UserName, setup.Password)
		So(err, ShouldBeNil)
		Convey("Test_Table_with_decimal128_not_null:", func() {
			s, err := db.RunScript("c1=decimal128(take(1.231284,10),0);c2=decimal128(take('1.123123123123123123123123123123123123123',10),30);t=table(c1,c2);t")
			So(err, ShouldBeNil)
			result := s.(*model.Table)
			So(result.Rows(), ShouldEqual, 10)
			c1 := result.GetColumnByName("c1").Data.Value()
			c2 := result.GetColumnByName("c2").Data.Value()
			for i := 0; i < len(c1); i++ {
				So(c1[i].(*model.Decimal128).Scale, ShouldEqual, 0)
				So(c2[i].(*model.Decimal128).Scale, ShouldEqual, 30)
				So(c1[i].(*model.Decimal128).Value, ShouldEqual, "1")
				So(c2[i].(*model.Decimal128).Value, ShouldEqual, "1.123123123123123123123123123123")
			}
		})
		Convey("Test_Table_with_decimal128_all_null:", func() {
			s, err := db.RunScript("c1=take(decimal128(NULL, 0), 1000);c2=take(decimal128(NULL, 30), 1000);t=table(c1,c2);t")
			So(err, ShouldBeNil)
			result := s.(*model.Table)
			So(result.Rows(), ShouldEqual, 1000)
			c1 := result.GetColumnByName("c1").Data
			c2 := result.GetColumnByName("c2").Data
			for i := 0; i < len(c1.Value()); i++ {
				assert.Equal(t, c1.Value()[i].(*model.Decimal128).Scale, int32(0))
				assert.Equal(t, c2.Value()[i].(*model.Decimal128).Scale, int32(30))
				So(c1.Get(i).IsNull(), ShouldBeTrue)
				So(c2.Get(i).IsNull(), ShouldBeTrue)
				assert.Equal(t, c1.Value()[i].(*model.Decimal128).String(), "")
				assert.Equal(t, c2.Value()[i].(*model.Decimal128).String(), "")
			}
		})
		So(db.Close(), ShouldBeNil)
	})
...
func Test_Table_UpLoad_DataType_string(t *testing.T) {
	...
	Convey("Test_Table_upload_with_string:", t, func() {
		...
		Convey("Test_Table_with_string:", func() {
			...
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"col"}, []*model.Vector{model.NewVector(col)})
			So(err, ShouldBeNil)
...
func Test_Table_UpLoad_DataType_int(t *testing.T) {
	...
	Convey("Test_Table_upload_with_int:", t, func() {
		...
		Convey("Test_Table_with_int:", func() {
			...
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"col", "ss"}, []*model.Vector{model.NewVector(col), model.NewVector(col1)})
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": tb})
			So(err, ShouldBeNil)
			res, _ := db.RunScript("s")
...
func Test_Table_UpLoad_DataType_decimal32(t *testing.T) {
	...
	Convey("Test_Table_upload_with_decimal32:", t, func() {
		...
		Convey("Test_Table_with_decimal32:", func() {
			...
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"c1"}, []*model.Vector{model.NewVector(col)})
			So(err, ShouldBeNil)
			fmt.Println("tb:", tb)
			_, err = db.Upload(map[string]model.DataForm{"s": tb})
			So(err, ShouldBeNil)
...
func Test_Table_UpLoad_DataType_decimal128(t *testing.T) {
	t.Parallel()
	Convey("Test_Table_upload_with_decimal128:", t, func() {
		db, err := api.NewSimpleDolphinDBClient(context.TODO(), setup.Address, setup.UserName, setup.Password)
		So(err, ShouldBeNil)
		Convey("Test_Table_with_decimal128:", func() {
			col, err := model.NewDataTypeListFromRawData(model.DtDecimal128, &model.Decimal128s{30, []string{"-1.123123123123123123123123123123123123123", "0", ""}})
			So(err, ShouldBeNil)
			colNull, err := model.NewDataTypeListFromRawData(model.DtDecimal128, &model.Decimal128s{0, []string{model.NullDecimal128Value, model.NullDecimal128Value, model.NullDecimal128Value}})
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"c1", "c2"}, []*model.Vector{model.NewVector(col), model.NewVector(colNull)})
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": tb})
			So(err, ShouldBeNil)
			res, _ := db.RunScript("t=table(decimal128(['-1.123123123123123123123123123123', '0', NULL], 30) as c1, [decimal128(NULL,0),decimal128(NULL,0),decimal128(NULL,0)] as c2);eqObj(t.values(),s.values())")
			So(res.(*model.Scalar).Value(), ShouldBeTrue)
		})
		So(db.Close(), ShouldBeNil)
	})
...
func Test_Table_UpLoad_DataType_complex(t *testing.T) {
	...
	Convey("Test_Table_upload_with_complex:", t, func() {
		...
		Convey("Test_Table_with_complex:", func() {
			...
			So(err, ShouldBeNil)
			tb, err := model.NewTable([]string{"complex"}, []*model.Vector{model.NewVector(col)})
			So(err, ShouldBeNil)
			_, err = db.Upload(map[string]model.DataForm{"s": tb})
			So(err, ShouldBeNil)
			res, _ := db.RunScript("s")
...
func Test_Table_UpLoad_big_array(t *testing.T) {
	...
	Convey("Test_Table_upload_with_big_array:", t, func() {
		...
		So(err, ShouldBeNil)
		tb, err := model.NewTable([]string{"int_v", "str_v", "all_null"}, []*model.Vector{model.NewVector(col), model.NewVector(col1), model.NewVector(allnullv)})
		So(err, ShouldBeNil)
		_, err = db.Upload(map[string]model.DataForm{"s": tb})
		So(err, ShouldBeNil)
		res, _ := db.RunScript("s")
...
func Test_Table_UpLoad_empty_table(t *testing.T) {
	...
	Convey("Test_Table_upload_empty_table", t, func() {
		...
		for i := 0; i < len(testTypes); i++ {
			...
			tb, err := model.NewTable([]string{"col"}, []*model.Vector{model.NewVector(model.NewEmptyDataTypeList(dataTypes[i], 0))})
			So(err, ShouldBeNil)
...
func Test_Table(t *testing.T) {
	Convey("Test_table", t, func() {
		...
		vct2 := model.NewVector(dtStr)

		tb, err := model.NewTable([]string{"col1", "col2"}, []*model.Vector{vct, vct2})
		So(err, ShouldBeNil)
		So(tb, ShouldNotBeNil)
		So(tb.Rows(), ShouldEqual, 2)
		So(tb.Columns(), ShouldEqual, 2)

		tb, err = model.NewTable([]string{}, []*model.Vector{})
		So(err, ShouldBeNil)
		So(tb.Rows(), ShouldEqual, 0)
