...
type SdHandler struct {
	...
	Msg1Total int
	Msg2Total int
	Res1Data  []*model.Vector
	Res2Data  []*model.Vector
...
func (s *SdHandler) DoEvent(msg streaming.IMessage) {
	...
	if sym == "msg1" {
		...
		for i := 0; i < len(s.Coltype1); i++ {
			...
			AssertNil(err)
			AssertNil(s.Res1Data[i].Append(dt))
		}
		...
		for i := 0; i < len(s.Coltype2); i++ {
			...
			AssertNil(err)
			AssertNil(s.Res2Data[i].Append(dt))
		}
	}
	s.Lock.Unlock()
}
	...
	if sym == "msg1" {
		...
		for i := 0; i < len(s.Coltype1); i++ {
			if i != 3 {
				...
				s.Res1Data[i].Append(val)
			} else {
				val := ret.GetValue(i).(*model.Vector)
				s.Res1Data[i].AppendVectorValue(val.GetVectorValue(0))
			}
		}
		...
		for i := 0; i < len(s.Coltype2); i++ {
			if i != 3 {
				val := ret.GetValue(i).(*model.Scalar).DataType
				s.Res2Data[i].Append(val)
			} else {
				val := ret.GetValue(i).(*model.Vector)
				s.Res2Data[i].AppendVectorValue(val.GetVectorValue(0))
			}
		}
	}
	s.Lock.Unlock()
...
func CreateStreamDeserializer(conn api.DolphinDB, tbname string) (SdHandler, SdBatchHandler) {
	...
	plock1 := &lock1
	plock2 := &lock2
	sh := SdHandler{Sd: *sd, Msg1Total: 0, Msg2Total: 0, Res1Data: args1, Res2Data: args2, Coltype1: ex_types1, Coltype2: ex_types2, Lock: plock1}
	sbh := SdBatchHandler{Sd: *sd, Msg1Total: 0, Msg2Total: 0, Res1Data: args1, Res2Data: args2, Coltype1: ex_types1, Coltype2: ex_types2, Lock: plock2}
...
func CreateStreamDeserializer_av(conn api.DolphinDB, tbname string, dataType model.DataTypeByte, vecVal string) (SdHandler_av, SdBatchHandler_av) {
	...
	plock1 := &lock1
	plock2 := &lock2
	sh := SdHandler_av{Sd: *sd, Msg1Total: 0, Msg2Total: 0, Res1Data: args1, Res2Data: args2, Coltype1: ex_types1, Coltype2: ex_types2, Lock: plock1}
	sbh := SdBatchHandler_av{Sd: *sd, Msg1Total: 0, Msg2Total: 0, Res1Data: args1, Res2Data: args2, Coltype1: ex_types1, Coltype2: ex_types2, Lock: plock2}
	fmt.Println("create handler successfully.")
	return sh, sbh
}
