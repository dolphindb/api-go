...
var host2 = helper.GetRandomClusterAddress()
var gpcConn_r, _ = api.NewSimpleDolphinDBClient(context.TODO(), host2, setup.UserName, setup.Password)

func TestNewGoroutinePooledClient_exception(t *testing.T) {
	var gpc_r = streaming.NewGoroutinePooledClient(setup.IP, setup.Reverse_subPort)
	Convey("TestNewGoroutinePooledClient_TableName_empty", t, func() {
		st, receive := helper.CreateStreamingTableWithRandomName(gpcConn_r)
		...
		So(err, ShouldBeNil)
		So(err.Error(), ShouldEqual, "the table name must not be empty")
		helper.ClearStreamTable(host2, st)
		helper.ClearStreamTable(host2, receive)
	})
	gpc_r.Close()
...
func TestNewGoroutinePooledClient_subscribe_ex_ubsubscribe_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_subscribe_ex_ubsubscribe", t, func() {
		...
		So(err, ShouldBeNil)
		helper.WaitData(gpcConn_r, receive, 1000)
		...
		re := tmp1.(*model.Table)
		ex := tmp2.(*model.Table)
		helper.CheckmodelTableEqual(re, ex, 0)
...
func TestNewGoroutinePooledClient_subscribe_offset_negative_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_subscribe_offset_negative", t, func() {
		...
		So(err, ShouldBeNil)
		helper.WaitData(gpcConn_r, receive, 2000)
...
func TestNewGoroutinePooledClient_subscribe_offset_10_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_subscribe_offset_10", t, func() {
		...
		_, err = gpcConn_r.RunScript("n=1000;t=table(1..n as tag,now()+1..n as ts,rand(100.0,n) as data);" + "" + st + ".append!(t)")
		So(err, ShouldBeNil)
		helper.WaitData(gpcConn_r, receive, 1090)
		tmp1, err := gpcConn_r.RunScript("select * from " + receive + " order by tag")
		So(err, ShouldBeNil)
...
func TestNewGoroutinePooledClient_subscribe_filter_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_subscribe_filter", t, func() {
		...
		helper.CheckmodelTableEqual(tmp1.(*model.Table), tmp3.(*model.Table), 1000)
		...
		// helper.CheckmodelTableEqual(tmp1.(*model.Table), tmp3.(*model.Table), 2000)
...
func TestNewGoroutinePooledClient_tableName_handler_offset_reconnect_success_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_tableName_handler_offset_reconnect_success", t, func() {
		...
		_, err = gpcConn_r.RunScript("stopPublishTable('" + strings.Split(host2, ":")[0] + "'," + strings.Split(host2, ":")[1] + ",'" + st + "','subTradesTable')")
		So(err, ShouldBeNil)
		time.Sleep(2 * time.Second)
		...
		helper.WaitData(gpcConn_r, receive, 2000)
		res, _ := gpcConn_r.RunScript("res = select * from " + receive + " order by tag,ts;ex = select * from " + st + " order by tag,ts;all(each(eqObj, ex.values(), res.values()))")
...
func TestNewGoroutinePooledClient_batchSize_throttle2_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_batchSize_throttle2", t, func() {
		...
		_, err = gpcConn_r.RunScript("n=100;t=table(1..n as tag,now()+1..n as ts,rand(100.0,n) as data);" + "" + st + ".append!(t)")
		So(err, ShouldBeNil)
		helper.WaitData(gpcConn_r, receive, 200)
		tmp1, err := gpcConn_r.RunScript("select * from " + receive + " order by tag,ts,data")
		So(err, ShouldBeNil)
		...
		helper.CheckmodelTableEqual(re1, ex, 0)
		helper.ClearStreamTable(host2, st)
		helper.ClearStreamTable(host2, receive)
	})
	gpc_r.Close()
...
func TestNewGoroutinePooledClient_msgAsTable_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_msgAsTable", t, func() {
		st, receive := helper.CreateStreamingTableWithRandomName(gpcConn_r)
		handler := helper.MessageHandler_table{
			Receive: receive,
			Conn:    gpcConn_r,
		}
		req1 := &streaming.SubscribeRequest{
...
func TestNewGoroutinePooledClient_subscribe_with_StreamDeserializer_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_subscribe_onehandler_with_StreamDeserializer", t, func() {
		sdhandler, _ := helper.CreateStreamDeserializer(gpcConn_r, "SDoutTables_gpc_r")
		req1 := &streaming.SubscribeRequest{
			Address:    host2,
		...
		for {
			time.Sleep(1 * time.Second)
			if sdhandler.Msg1Total+sdhandler.Msg2Total == targetows {
				break
			} else {
				fmt.Println(sdhandler.Msg1Total + sdhandler.Msg2Total)
	...
	Convey("TestNewGoroutinePooledClient_subscribe_batchHandler_with_StreamDeserializer", t, func() {
		_, sdBatchHandler1 := helper.CreateStreamDeserializer(gpcConn_r, "SDoutTables_gpc_r")
		req1 := &streaming.SubscribeRequest{
			Address:      host2,
			TableName:    "SDoutTables_gpc_r",
			ActionName:   "testStreamDeserializer",
		...
		for {
			time.Sleep(1 * time.Second)
			if sdBatchHandler1.Msg1Total+sdBatchHandler1.Msg2Total == targetows {
				break
			} else {
				fmt.Println(sdBatchHandler1.Msg1Total + sdBatchHandler1.Msg2Total)
			}
		}
		err = gpc_r.UnSubscribe(req1)
		So(err, ShouldBeNil)

		res_tab1, err := model.NewTable([]string{"datetimev", "timestampv", "sym", "price1", "price2"}, sdBatchHandler1.Res1Data)
		So(err, ShouldBeNil)
		res_tab2, err := model.NewTable([]string{"datetimev", "timestampv", "sym", "price1"}, sdBatchHandler1.Res2Data)
		So(err, ShouldBeNil)
...
func TestNewGoroutinePooledClient_unsubscribe_in_doEvent_r(t *testing.T) {
	...
	Convey("TestNewGoroutinePooledClient_unsubscribe_in_doEvent", t, func() {
		st, receive := helper.CreateStreamingTableWithRandomName(gpcConn_r)
		handler := helper.MessageHandler_unsubscribeInDoEvent{
			SubType:   "gpc",
			SubClient: gpc_r,
...
func TestNewGoroutinePooledClient_subscribe_allTypes_r(t *testing.T) {
	...
	for _, data := range testDatas {
		Convey("TestNewGoroutinePooledClient_subscribe_oneHandler_alltypes", t, func() {
			...
			So(err, ShouldBeNil)
			st, re := helper.CreateStreamingTableWithRandomName_allTypes(gpcConn_r, data.Dt, data.VecVal)
		...
		Convey("TestNewGoroutinePooledClient_subscribe_oneHandler_arrayVector", t, func() {
			_, err := gpcConn_r.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					"try{ dropStreamTable(`st1);}catch(ex){};")
			So(err, ShouldBeNil)
			st, re := helper.CreateStreamingTableWithRandomName_av(gpcConn_r, data.Dt, data.VecVal)
			appenderOpt := &api.TableAppenderOption{
				TableName: re,
				Conn:      gpcConn_r,
			}
		...
		Convey("TestNewGoroutinePooledClient_subscribe_oneHandler_with_StreamDeserializer_arrayVector", t, func() {
			...
			_, err := gpcConn_r.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					...
					"try{ undef(`table2, SHARED);}catch(ex){};go")
			So(err, ShouldBeNil)
			sdhandler, _ := helper.CreateStreamDeserializer_av(gpcConn_r, tbname, data.Dt, data.VecVal)
			req1 := &streaming.SubscribeRequest{
				Address:    host2,
				TableName:  tbname,
				ActionName: "testStreamDeserializer",
		...
		Convey("TestNewGoroutinePooledClient_subscribe_batchHandler_with_StreamDeserializer_arrayVector", t, func() {
			...
			_, err := gpcConn_r.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					...
					"try{ undef(`table2, SHARED);}catch(ex){};go")
			So(err, ShouldBeNil)
			_, sdBatchHandler := helper.CreateStreamDeserializer_av(gpcConn_r, tbname, data.Dt, data.VecVal)
			req1 := &streaming.SubscribeRequest{
				Address:      host2,
				TableName:    tbname,
				ActionName:   "testStreamDeserializer",
			...
			for {
				time.Sleep(1 * time.Second)
				if sdBatchHandler.Msg1Total+sdBatchHandler.Msg2Total == targetows {
					break
				}
			}
			err = gpc_r.UnSubscribe(req1)
...
func TestGoroutinePooledClient_subscribe_with_SCRAM_user_r(t *testing.T) {
	_, err := gpcConn_r.RunScript("try{deleteUser('scramUser')}catch(ex){};go;createUser(`scramUser, `123456, authMode='scram')")
	if err != nil {
	...
	Convey("TestGoroutinePooledClient_subscribe_SCRAM_user", t, func() {
		...
		So(err, ShouldBeNil)
		helper.WaitData(gpcConn_r, receive, 1000)
		ret, err := gpcConn_r.RunScript("res = select * from " + st + " order by ts;ex = select * from " + receive + " order by ts;all(each(eqObj, res.values(), ex.values()))")
		So(err, ShouldBeNil)
