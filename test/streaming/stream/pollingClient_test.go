...
var host5 = helper.GetRandomClusterAddress()
...
func TestSubscribe_exception(t *testing.T) {
	...
	Convey("Test_subscribe_exception", t, func() {
		...
		st, receive := helper.CreateStreamingTableWithRandomName(pcConn)
		...
		Convey("Test_subscribe_AllowExists", func() {
			...
			helper.ClearStreamTable(host5, st)
			helper.ClearStreamTable(host5, receive)
		})
	})
	pc.Close()
...
func TestPollingClient_subscribe_filter(t *testing.T) {
	...
	Convey("TestPollingClient_subscribe_filter", t, func() {
		...
		res, _ := pcConn.RunScript("res = select * from " + receive + " where tag between 1:1000 order by tag,ts;ex = select * from " + st + " where tag between 1:1000 order by tag,ts;all(each(eqObj, ex.values(), res.values(), 5))")
		So(res.(*model.Scalar).Value().(bool), ShouldBeTrue)

		res, _ = pcConn.RunScript("res = select * from " + receive + " where tag between 2001:3000 order by tag,ts;ex = select * from " + st + " where tag between 2001:3000 order by tag,ts;all(each(eqObj, ex.values(), res.values(),5))")
		So(res.(*model.Scalar).Value().(bool), ShouldBeTrue)
...
func TestPollingClient_subscribe_with_StreamDeserializer(t *testing.T) {
	...
	Convey("TestPollingClient_subscribe_with_StreamDeserializer", t, func() {
		sdhandler, _ := helper.CreateStreamDeserializer(pcConn, "SDoutTables_pc")
		req1 := &streaming.SubscribeRequest{
			Address:    host5,
...
func TestNewPollingClient_subscribe_allTypes(t *testing.T) {
	...
	for _, data := range testDatas {
		Convey("TestNewPollingClient_subscribe_oneHandler_arrayVector", t, func() {
			...
			So(err, ShouldBeNil)
			st, re := helper.CreateStreamingTableWithRandomName_allTypes(pcConn, data.Dt, data.VecVal)
		...
		Convey("TestNewPollingClient_subscribe_oneHandler_arrayVector", t, func() {
			_, err := pcConn.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					"try{ dropStreamTable(`st1);}catch(ex){};")
			So(err, ShouldBeNil)
			st, re := helper.CreateStreamingTableWithRandomName_av(pcConn, data.Dt, data.VecVal)
			appenderOpt := &api.TableAppenderOption{
				TableName: re,
				Conn:      pcConn,
			}
		...
		Convey("TestNewPollingClient_subscribe_oneHandler_with_StreamDeserializer_arrayVector", t, func() {
			...
			_, err := pcConn.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					...
					"try{ undef(`table2, SHARED);}catch(ex){};go")
			So(err, ShouldBeNil)
			sdhandler, _ := helper.CreateStreamDeserializer_av(pcConn, tbname, data.Dt, data.VecVal)
			req1 := &streaming.SubscribeRequest{
				Address:    host5,
				TableName:  tbname,
				ActionName: "testStreamDeserializer",
		...
		Convey("TestNewPollingClient_subscribe_batchHandler_with_StreamDeserializer_arrayVector", t, func() {
			...
			_, err := pcConn.RunScript(
				"try{ dropStreamTable(`st2);}catch(ex){};" +
					...
					"try{ undef(`table2, SHARED);}catch(ex){};go")
			So(err, ShouldBeNil)
			_, sdBatchHandler := helper.CreateStreamDeserializer_av(pcConn, tbname, data.Dt, data.VecVal)
			req1 := &streaming.SubscribeRequest{
				Address:      host5,
				TableName:    tbname,
				ActionName:   "testStreamDeserializer",
...
func TestNewPollingClient_subscribe_with_SCRAM_user(t *testing.T) {
	_, err := pcConn.RunScript("try{deleteUser('scramUser')}catch(ex){};go;createUser(`scramUser, `123456, authMode='scram')")
	if err != nil {
	...
	Convey("TestNewPollingClient_subscribe_SCRAM_user", t, func() {
		...
		ret, err := pcConn.RunScript("res = select * from " + st + " order by ts;ex = select * from " + receive + " order by ts;all(each(eqObj, res.values(), ex.values()))")
		So(err, ShouldBeNil)
