...
func CreateStreamingTable() {
	...
	AssertNil(err)
	script := "try{ dropStreamTable(`tradesTable) }catch(ex){};" +
...
func TestSubsribe_size(t *testing.T) {
	Convey("TestSubsribe_size", t, func() {
		...
		poller2, err := pc.Subscribe(req2)
		So(err, ShouldBeNil)
		for i := 0; i < 10; i++ {
			_, err = ddb.RunScript("dataNum=5000;insert into tradesTable values(take(now(), dataNum), take(`000905`600001`300201`000908`600002, dataNum), rand(1000,dataNum)/10.0);")
			So(err, ShouldBeNil)
			msg1 := poller1.Poll(10000, 10000)
			msg2 := poller2.Poll(10000, 1000)
			if msg1 == nil {
				continue
			} else if len(msg1) > 0 {
			...
			if msg2 == nil {
				...
				So(len(msg2), ShouldBeGreaterThanOrEqualTo, 1000)
			}
		}
...
func TestSubsribe_take(t *testing.T) {
	Convey("TestSubsribe_take", t, func() {
		...
		So(err, ShouldBeNil)
		retimev := IMessage.GetValue(0).(*model.Scalar).String()
		resymbol := IMessage.GetValue(1).(*model.Scalar).String()
		repricev := IMessage.GetValue(2).(*model.Scalar).String()
		...
		So(retimev, ShouldEqual, "timestamp("+extimev+")")
		So(resymbol, ShouldEqual, "string("+exsymbol+")")
		So(repricev, ShouldEqual, "double("+expricev+")")
		So(retimev1, ShouldEqual, "timestamp("+extimev+")")
		So(resymbol1, ShouldEqual, "string("+exsymbol+")")
		So(repricev1, ShouldEqual, "double("+expricev+")")
		err = pc.UnSubscribe(req)
		So(err, ShouldBeNil)
		So(ddb.Close(), ShouldBeNil)
	})
