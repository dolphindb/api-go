package api

import (
	"context"
	"net"
	"os"
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"
)

const testAddress = "127.0.0.1:3003"

func TestAccount(t *testing.T) {
	db, err := NewDolphinDBClient(context.TODO(), testAddress, nil)
	assert.Nil(t, err)

	err = db.Connect()
	assert.Nil(t, err)

	loginReq := new(LoginRequest).
		SetPassword("password").
		SetUserID("user")
	err = db.Login(loginReq)
	assert.Nil(t, err)

	err = db.Logout()
	assert.Nil(t, err)
}

func TestMain(m *testing.M) {
	exit := make(chan bool)

	ln, err := net.Listen("tcp", testAddress)
	if err != nil {
		return
	}
	go func() {
		for !isExit(exit) {
			conn, err := ln.Accept()
			if err != nil {
				return
			}

			go handleData(conn)
		}

		ln.Close()
	}()

	exitCode := m.Run()

	close(exit)

	os.Exit(exitCode)
}

func handleData(conn net.Conn) {
	defer conn.Close()

	const (
		boolScalarTrueResponse  = "20267359 1 1\nOK\n\x01\x00\x01"
		boolScalarFalseResponse = "20267359 1 1\nOK\n\x01\x00\x00"
		scramUnavailableError   = "20267359 0 1\nCan't recognize function name scramClientFirst\n"
		successResponse         = "20267359 0 1\nOK\n"
	)

	res := make([]byte, 0)
	for {
		buf := make([]byte, 512)
		l, err := conn.Read(buf)
		if err != nil {
			continue
		}

		res = append(res, buf[0:l]...)
		script := string(res)
		length := len(res)
		var resp []byte
		switch {
		case strings.Contains(script, "scramClientFirst"):
			resp = []byte(scramUnavailableError)
		case strings.Contains(script, "existsDatabase"):
			resp = []byte(boolScalarFalseResponse)
		case strings.Contains(script, "existsTable"):
			resp = []byte(boolScalarFalseResponse)
		case strings.Contains(script, "isNodeInitialized"):
			resp = []byte(boolScalarTrueResponse)
		case strings.Contains(script, "login"):
			resp = []byte(successResponse)
		case strings.Contains(script, "logout()"):
			resp = []byte(successResponse)
		case strings.Contains(script, "schema(loadTable") || ((length == 62 || length == 57) && strings.Index(script, "schema") > 0):
			resp = []byte{}
		case length == 151 || length == 149 || length == 141:
			resp = []byte{0x32, 0x30, 0x32, 0x36, 0x37, 0x33, 0x35, 0x39, 0x20, 0x31, 0x20, 0x31, 0x0a, 0x4f, 0x4b, 0x0a, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00}
		case strings.Contains(script, "select * from") || length == 43 || length == 50:
			resp = []byte{0x31, 0x38, 0x32, 0x33, 0x32, 0x38, 0x39, 0x31, 0x37, 0x36, 0x20, 0x31, 0x20, 0x31, 0x0a, 0x4f,
				0x4b, 0x0a, 0x00, 0x06, 0x03, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x69, 0x64, 0x00, 0x78, 0x00, 0x12, 0x01, 0x03,
				0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x58, 0x4f, 0x4d, 0x00, 0x47, 0x53, 0x00, 0x41, 0x41, 0x50, 0x4c, 0x00, 0x10, 0x01,
				0x03, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x66, 0x66, 0x66, 0x66, 0x66, 0x86, 0x59, 0x40, 0x33, 0x33, 0x33, 0x33, 0x33,
				0xb3, 0x40, 0x40, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x52, 0x40}
		case isMockSuccessScript(script) || isSuccessRequest(length):
			resp = []byte{0x32, 0x30, 0x32, 0x36, 0x37, 0x33, 0x35, 0x39, 0x20, 0x30, 0x20, 0x31, 0x0a, 0x4f, 0x4b, 0x0a}
		case strings.Contains(script, "getClusterLiveDataNodes"):
			resp = []byte{0x32, 0x38, 0x35, 0x36, 0x30, 0x35, 0x34, 0x32, 0x36, 0x33, 0x20, 0x31, 0x20, 0x31, 0x0a, 0x4f, 0x4b, 0x0a, 0x12,
				0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x31, 0x32, 0x37, 0x2e, 0x30, 0x2e, 0x30, 0x2e, 0x31, 0x3a, 0x33,
				0x30, 0x30, 0x33, 0x3a, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x33, 0x30, 0x30, 0x33, 0x00}
		case strings.Contains(script, "schema(") || (length == 35 && strings.Index(script, "schema") > 0):
			resp = []byte{}
		case strings.Contains(script, "append!") || strings.Contains(script, "tableInsert") || length == 51 || length == 55:
			resp = []byte{0x32, 0x30, 0x32, 0x36, 0x37, 0x33, 0x35, 0x39, 0x20, 0x31, 0x20, 0x31, 0x0a, 0x4f, 0x4b, 0x0a, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00}
		}

		if len(resp) > 0 {
			_, err = conn.Write(resp)
			if err != nil {
				return
			}

			res = make([]byte, 0)
		}
	}
}

var successLength = []int{33, 48, 95, 67, 52, 57, 54, 63, 25, 27, 40, 42, 45, 44, 78, 89, 90, 79, 87, 64, 53, 34, 41, 49, 60, 150, 30, 59}

func isSuccessRequest(l int) bool {
	for _, v := range successLength {
		if v == l {
			return true
		}
	}

	return false
}

func isMockSuccessScript(script string) bool {
	successScripts := []string{
		"createTable",
		"createPartitionedTable",
		"typestr",
	}
	for _, v := range successScripts {
		if strings.Contains(script, v) {
			return true
		}
	}

	return false
}

func isExit(exit <-chan bool) bool {
	select {
	case <-exit:
		return true
	default:
		return false
	}
}
