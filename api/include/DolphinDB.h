...
class Guid {
 ...
 private:
  ...
  inline unsigned char hexPairToChar(char a, char b) const {
...
class Constant {
 public:
  ...
  inline bool isVector() const {
    return getForm() >= DF_VECTOR && getForm() <= DF_MATRIX;
  }
  ...
  virtual char getBool() const {
    throw RuntimeException("The object can't be converted to boolean scalar.");
  }
  virtual char getChar() const {
    throw RuntimeException("The object can't be converted to char scalar.");
  }
  virtual short getShort() const {
    throw RuntimeException("The object can't be converted to short scalar.");
  }
  virtual int getInt() const {
    throw RuntimeException("The object can't be converted to int scalar.");
  }
  virtual long long getLong() const {
    throw RuntimeException("The object can't be converted to long scalar.");
  }
  virtual INDEX getIndex() const {
    throw RuntimeException("The object can't be converted to index scalar.");
  }
  virtual float getFloat() const {
    throw RuntimeException("The object can't be converted to float scalar.");
  }
  virtual double getDouble() const {
    throw RuntimeException("The object can't be converted to double scalar.");
  }
  ...
  virtual const Guid getUuid() const {
  ...
  virtual ConstantSP getWindow(INDEX colStart, int colLength, INDEX rowStart,
                               int rowLength) const {
    return getValue();
  }
  ...
  virtual const long long* getLongConst(INDEX start, int len,
                                        long long* buf) const {
  ...
  virtual const double* getDoubleConst(INDEX start, int len,
                                       double* buf) const {
  ...
  virtual int serialize(char* buf, int bufSize, INDEX indexStart, int offset,
    ...
    throw RuntimeException("serialize method not supported");
  }
  virtual IO_ERR deserialize(DataInputStream* in, INDEX indexStart,
                             INDEX targetNumElement, INDEX& numElement) {
    throw RuntimeException("deserialize method not supported");
  }
  ...
  virtual INDEX itemCount() const {
    return getForm() == DF_MATRIX ? columns() : size();
  }
  ...
  virtual ConstantSP keys() const {
    throw RuntimeException("keys method not supported");
  }
  virtual ConstantSP values() const {
    throw RuntimeException("values method not supported");
  }
...
class Vector : public Constant {
 public:
  ...
  virtual INDEX reserve(INDEX capacity) {
    throw RuntimeException("Vector::reserve method not supported");
  }
  ...
  virtual bool append(const ConstantSP& value) {
    return append(value, value->size());
  }
  ...
  virtual ConstantSP getSubVector(INDEX start, INDEX length,
  ...
  virtual long long getAllocatedMemory(INDEX size) const {
    return Constant::getAllocatedMemory();
  }
...
class Set : public Constant {
 public:
  ...
  virtual void contain(const ConstantSP& target,
                       const ConstantSP& resultSP) const = 0;
...
class Table : public Constant {
 public:
  ...
  virtual ConstantSP getColumn(const string& qualifier,
                               const string& name) const = 0;
  virtual ConstantSP getColumn(INDEX index) const = 0;
  virtual ConstantSP getColumn(const string& name,
                               const ConstantSP& rowFilter) const = 0;
  virtual ConstantSP getColumn(const string& qualifier, const string& name,
                               const ConstantSP& rowFilter) const = 0;
  virtual ConstantSP getColumn(INDEX index,
                               const ConstantSP& rowFilter) const = 0;
  ...
  virtual void drop(vector<int>& columns) {
    throw RuntimeException("Table::drop() not supported");
  }
  virtual bool update(vector<ConstantSP>& values, const ConstantSP& indexSP,
                      vector<string>& colNames, string& errMsg) = 0;
  virtual bool append(vector<ConstantSP>& values, INDEX& insertedRows,
                      string& errMsg) = 0;
...
class DFSChunkMeta : public Constant {
 public:
  DFSChunkMeta(const string& path, const Guid& id, int version, int size,
               CHUNK_TYPE chunkType, const vector<string>& sites,
               long long cid);
  DFSChunkMeta(const string& path, const Guid& id, int version, int size,
               CHUNK_TYPE chunkType, const string* sites, int siteCount,
               long long cid);
  ...
  virtual ConstantSP getValue() const {
    return new DFSChunkMeta(path_, id_, version_, size_, (CHUNK_TYPE)type_,
                            sites_, replicaCount_, cid_);
  }
...
class ConstantMarshall {
 public:
  ...
  virtual bool start(const char* requestHeader, size_t headerSize,
                     const ConstantSP& target, bool blocking, IO_ERR& ret) = 0;
...
class EXPORT_DECL DBConnection {
 public:
  ...
  /**
   * Connect to the specified DolphinDB server. If userId and password are
   * specified, authentication will be performed along with connecting. If one
   * would send userId and password in encrypted mode, please use the login
   * function for authentication separately.
   */
  bool connect(const string& hostName, int port, const string& userId = "",
               const string& password = "");

  /**
   * Log onto the DolphinDB server using the given userId and password. If the
   * parameter enableEncryption is true, the client obtains a public key from
   * the server and then encrypts the userId and password using the public key.
   * If the parameter enableEncryption is false, userId and password are
  ...
  /**
   * Run the script on the DolphinDB server and return the result to the
   * client.If nothing returns, the function returns a void object. If error is
   * raised on the server, the function throws an exception.
   */
  ConstantSP run(const string& script);

  /**
   * Run the given function on the DolphinDB server using the local objects as
   * the arguments for the function and return the result to the client. If
   * nothing returns, the function returns a void object. If error is raised on
   * the server, the function throws an exception.
   */
  ConstantSP run(const string& funcName, vector<ConstantSP>& args);

  /**
   * upload a local object to the DolphinDB server and assign the given name in
   * the session.
   */
  void upload(const string& name, const ConstantSP& obj);

  /**
   * upload multiple local objects to the DolphinDB server and assign each
   * object the given name in the session.
   */
  ...
  /**
   * It is required to call initialize function below before one uses the
   * DolphinDB API.
   */
  static void initialize();

 private:
  ConstantSP run(const string& script, const string& scriptType,
