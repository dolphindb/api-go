...
func TestConstant_IsForm(t *testing.T) {
	...
	p1 := conn.Run("1+1")
	if !p1.IsScalar() {
		t.Error("IsScalar Error")
	}

	p2 := conn.Run("5 4 8")
	if !p2.IsArray() {
		t.Error("IsArray Error")
	}

	p3 := conn.Run("3:5")
	if !p3.IsPair() {
		t.Error("IsPair Error")
	}

	p4 := conn.Run("1..6$2:3")
	if !p4.IsMatrix() {
		t.Error("IsMatrix Error")
	}
...
func TestgetSlice(t *testing.T) {
	...
	p1 := p.GetIntSlice()
	if len(p1) != 10 {
		t.Error("GetIntSlice Error")
	}
	p2 := p.GetShortSlice()
	if len(p2) != 10 {
		t.Error("GetShortSlice Error")
	}
	p3 := p.GetLongSlice()
	if len(p3) != 10 {
		t.Error("GetLongSlice Error")
	}
	p4 := p.GetBoolSlice()
	if len(p4) != 10 {
		t.Error("GetBoolSlice Error")
	}
	p5 := p.GetFloatSlice()
	if len(p5) != 10 {
		t.Error("GetFloatSlice Error")
	}
	p6 := p.GetDoubleSlice()
	if len(p6) != 10 {
		t.Error("GetDoubleSlice Error")
	}
	p7 := p.GetStringSlice()
	if len(p7) != 10 {
		t.Error("GetStringSlice Error")
	}
	//	p1 := p.GetIntSlice();
